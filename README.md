...
Además, el cliente envía cada batch de apuestas en una conexión distinta. Esto permite que el servidor procese batches de diferentes agencias simultáneamente, sin necesidad de esperar a que una agencia termine de enviar todas sus apuestas antes de atender a otra.
...
El cliente puede enviar nuevos tipos de mensaje:
...
Por otro lado, el servidor tiene 3 códigos de respuesta:
...
Para superar esta limitación, se decidió utilizar la librería `multiprocessing`. De esta forma, por cada nueva conexión entrante, el servidor crea un nuevo proceso. Este proceso es el encargado de la comunicación con esa nueva conexión, que corresponde a una de las agencias.
...
Además, también era necesario sincronizar a los procesos para poder enviar los resultados de la lotería. Para este caso, una barrera (`Barrier`) fue la opción utilizada para sincronizar los procesos antes de enviar los resultados de la lotería. La barrera permite que todos los procesos esperen hasta que todos hayan alcanzado un punto específico antes de continuar. Esto es útil para asegurarse de que todos los procesos hayan terminado de enviar sus apuestas antes de calcular y enviar los resultados. Por lo tanto, se ponen los procesos a esperar en la barrera luego de solicitar al servidor los ganadores del sorteo.
