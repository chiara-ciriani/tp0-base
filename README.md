...
Además, se agrego un signal handler que captura la señal `signal.SIGTERM`. En caso que el servidor la reciba, se ejecutará la función establecida como handler `__handle_sigterm()`.
...
El protocolo de comunicación entre el cliente y el servidor se basa en el envío y recepción de paquetes estructurados. Cada mensaje consta de un encabezado (header) y un cuerpo (body). A continuación se describirá la estructura de cada parte del mensaje.
...
* **BET**: el Cliente envía un mensaje al Servidor que contiene una bet.
...
1. **Encabezado**:
    * **Longitud del mensaje**: 39 bytes (2 bytes)
...
La estructura es la siguiente:
* **Longitud del mensaje**: 2 bytes (uint16) que indican la longitud total del mensaje, incluyendo el resto del encabezado y el cuerpo.
* **ID de la agencia**: 1 byte (uint8) que indica el ID de la agencia que envía el mensaje.
 ...
 **Nota:** El tamaño de Apuestas depende de la cantidad de apuestas en el batch y de la longitud de cada apuesta. Todas siguen la estructura y la forma de deserializar ya mencionada previamente.
...
# Ejercicio N°7:
Previamente, el servidor simplemente almacenaba cada apuesta que los clientes le enviaban. En este ejercicio, se modificó el servidor para proceder con el sorteo una vez que todos los clientes le notifiquen la finalización del envío de todas las apuestas.

Además, inmediatamente después de enviar la notificación de finalización, los clientes consultan la lista de ganadores del sorteo correspondientes a su agencia.

## Configuración de `clients_amount`

Para que el servidor sepa cuándo ha terminado el sorteo, es necesario que conozca la cantidad total de clientes. Esto se logra configurando la variable `clients_amount` en el archivo `docker-compose-dev.yaml` generado por el script `generar-compose.sh`. En el script, se agregó la variable `clients_amount` en la sección de `environment` del servicio `server`. Esto es importante porque el servidor utiliza la variable para determinar cuándo ha recibido la notificación de finalización de todas las agencias. Una vez que todas las agencias han enviado sus apuestas, el servidor procede con el sorteo.
...
El cliente puede enviar un nuevo tipo de mensaje **WINNERS_REQUEST**, que se suma a los otros dos que el cliente podía ya enviar.

Los mensajes disponibles ahora son los siguientes:

1. **BATCH (0)**: el mensaje enviado contiene una cantidad de apuestas dadas por `batch: maxAmount`.
2. **BATCH_END (1)**: este mensaje es usado por el cliente para indicar que finalizó con el envio de apuestas. En otras palabras, es la notificación de finalización.
3. **WINNERS_REQUEST (2)**: este mensaje es usado por el cliente para consultar la lista de ganadores del sorteo correspondientes a su agencia.

### Formato de los mensajes

Tanto el mensaje **BATCH** como **BATCH_END**, ya introducidos en el ejercicio anterior, se mantienen con la misma estructura.

#### Mensaje **WINNERS_REQUEST**

Se envia del Cliente al Servidor solicitando la lista de ganadores al sorteo.

* **Header**:
    * 2 bytes: longitud del mensaje.
    * 1 byte: agency_id correspondiente a la agencia que envia el mensaje.
    * 1 byte: tipo de mensaje. En este caso, es un mensaje de tipo **WINNERS_REQUEST (3)**.

* **Payload**: No tiene.

## Mensajes del servidor

Por otro lado, el servidor tiene 4 códigos de respuesta:

1. **OK (0)**: las apuestas fueron almacenadas correctamente.
2. **ERROR (1)**: no se pudo almacenar la apuesta debido a algún error.
3. **SEND_WINNERS (2)**: el servidor está enviando los ganadores del sorteo correspondiente a la agencia destino.
4. **BET_NOT_FINISHED (3)**: el servidor indica que no todas las agencias han terminado de enviar sus apuestas, por lo que el sorteo aún no comenzó.

En los dos anteriores ejercicios, solamente se enviaban los primeros dos mensajes. Por lo tanto, del lado del Cliente era suficiente leer un sólo byte que representaba el tipo de mensaje. En este ejercicio fue necesario agregar un **header** a los mensajes del Servidor, al igual que como ya se hacia en el Cliente. Esto fue necesario debido a que el mensaje *SEND_WINNERS* envía en el payload los documentos de los ganadores. Por lo tanto, ya no basta solamente con el tipo del mensaje, sino que ahora se agrega la longitud del mensaje.

El nuevo formato de los mensajes del servidor es el siguiente:

* **Header**:
    * 2 bytes: longitud del mensaje.
    * 1 byte: tipo de mensaje.
* **Payload**: 
    * Opcional. Ninguno tiene salvo el mensaje **SEND_WINNERS** que contiene los documentos de todos los ganadores uno atrás del otro. 

En la siguiente tabla se puede obsevar el formato del header de los mensajes del servidor. En la tabla se puede observar que es igual al header de los mensajes del cliente, con la única diferencia que no tiene agency_id, al ser el servidor quien envía el mensaje.
...
Se envía del Servidor al Cliente como respuesta de éxito al procesamiento de la batch de apuestas recibida. Este mensaje no es nuevo, pero se actualizó el formato. Previamente, se enviaba un sólo byte para señalar el tipo de mensaje. Ahora los mensajes del Servidor también incluyen la longitud del mensaje, en caso que tengan un payload.
...
Se envía del Servidor al Cliente como respuesta de fracaso al procesamiento de la batch de apuestas recibida. Este mensaje no es nuevo, pero se actualizó el formato. Previamente, se enviaba un sólo byte para señalar el tipo de mensaje. Ahora los mensajes del Servidor también incluyen la longitud del mensaje, en caso que tengan un payload.
...
#### Mensaje **SEND_WINNERS**

Se envía del Servidor al Cliente con la lista de los ganadores del sorteo correspondientes a la agencia. Más específicamente, en el payload se enviarán los documentos correspondientes a los ganadores.

* **Header**:
    * 2 bytes: longitud del mensaje.
    * 1 byte: tipo de mensaje. En este caso, es un mensaje de tipo **SEND_WINNERS (2)**.
* **Payload**: 
    * Contiene los documentos de todos los ganadores uno atrás del otro. La longitud de cada uno está definido por `DOCUMENT_LEN` que son 4 bytes.

#### Mensaje **BET_NOT_FINISHED**

Se envía del Servidor al Cliente cuando el Cliente solicitó el envio de los ganadores pero todavía no se realizó el sorteo ya que no todas las agencias terminaron de mandar las apuestas.

* **Header**:
    * 2 bytes: longitud del mensaje. En este caso, siempre va a valer 1.
    * 1 byte: tipo de mensaje. En este caso, es un mensaje de tipo **BET_NOT_FINISHED (3)**.
* **Payload**: no tiene.

## Lectura de los mensajes

La única diferencia a los ejercicios anteriores es en la respuesta del servidor. 

Ahora, a los mensajes de respuesta se le agrega como primer byte (definido en `MSG_TYPE_LEN`) el largo del mensaje. Por lo tanto, del lado del cliente lo primero que se hace es leer un byte para saber el largo. En caso que sea un mensaje de tipo `SEND_WINNERS` (el único que tendrá mayor longitud que 1), el cliente seguirá leyendo en el socket. La forma de deserializar a los ganadores es sabiendo que cada documento ocupa 4 bytes (definido en `DOCUMENT_LEN`), debido al análisis que fue explicado previamente en la estructura de los mensajes.

## Flujo de comunicación

La primera parte se mantiene igual al ejercicio anterior:

1. Cada cliente se conecta con el servidor y mantiene una **única conexión**.
2. Durante esta conexión, el cliente envía mensajes de tipo **BATCH** que contienen un conjunto de apuestas. 
3. Entre cada batch, el cliente espera la confirmación del servidor (_OK(0)_)para evitar saturarlo. 
4. Una vez que el cliente termina de enviar todas las apuestas, envía un mensaje de tipo **BATCH_END**. 

En este caso, el mensaje **BATCH_END** es útil porque permite al servidor saber que el cliente ha finalizado, y el servidor agrega la agencia al conjunto `done_agencies`. Se eligió utilizar un conjunto (set) para evitar repeticiones.

Cuando la longitud de `done_agencies` es igual a `clients_amount` (la mencionada variable configurada en el script que representa la cantidad total de clientes), el servidor considera que todas las agencias han terminado y procede con el sorteo.

5. Después de finalizar el envío de apuestas, el cliente **cierra la conexión** y abre una **nueva conexión** con el servidor para enviar un mensaje de tipo **WINNERS_REQUEST**, solicitando los ganadores del sorteo.

El servidor puede responder de dos maneras:

6. **BET_NOT_FINISHED**: Si el servidor responde con este mensaje, significa que todavía no han terminado de enviar los resultados todas las agencias, por lo que el sorteo no ha comenzado. En este caso, el cliente cierra la conexión, realiza un _sleep_ y vuelve a realizar la consulta más tarde volviendo al paso 5.
7. **SEND_WINNERS**: Si el servidor responde con este mensaje, envía los documentos de los ganadores al cliente, finalizando el flujo.

## Cómo ejecutarlo

1. Generar el archivo compose
...
Por un lado, el servidor deberá esperar la notificación de las 5 agencias para considerar que se realizó el sorteo e imprimir por log: `action: sorteo | result: success`.
...
```
server   | 2025-03-24 16:55:32 INFO     action: receive_message | result: success | agency_id: 2 | message_type: MessageType.BATCH_END
server   | 2025-03-24 16:55:32 INFO     action: client_finished_sending_bets | result: success | agency: 2
server   | 2025-03-24 16:55:32 INFO     action: all_agencies_finished_sending_bets | result: success
server   | 2025-03-24 16:55:32 INFO     action: sorteo | result: success
```

En este ejemplo del servidor, se observa que la agencia 2 fue la última que terminó de enviar sus apuestas. Una vez que el servidor obtuvo su notificación de finalización, imprime un log que indica que todas las agencias terminaron. Por lo tanto, procede con el sorteo.

Luego, finalmente el servidor responde a los mensajes de solicitud de los ganadores a cada agencia. Por ejemplo, estos son los logs que muestran cómo le envía los ganadores al cliente 5.

```
server   | 2025-03-24 16:55:32 INFO     action: receive_message | result: success | agency_id: 5 | message_type: MessageType.WINNERS_REQUEST
...
Los resultados del sorteo, usando los archivos provistos por la cátedra, son los siguientes:
...
Se modificó el servidor para que permita aceptar conexiones y procesar mensajes en paralelo utilizando la librería `multiprocessing`. Esto se debe a que Python posee limitaciones propias del lenguaje al utilizar multithreading. En CPython, el Global Interpreter Lock (GIL) es un mutex que protege el acceso a los objetos de Python, impidiendo que múltiples threads ejecuten bytecodes de Python simultáneamente.

### Uso de `multiprocessing`

Para superar esta limitación, se decidió utilizar la librería `multiprocessing`. De esta forma, por cada nueva conexión entrante, el servidor crea un nuevo proceso. Este proceso es el encargado de la comunicación con esa nueva conexión, que corresponde a una de las agencias. De esta manera, el servidor es el responsable de aceptar nuevas conexiones mientras que el proceso es el encargado del resto de la lógica con el cliente.

### Sincronización al archivo de apuestas

Al paralelizar estas acciones y tener procesos separados por cada cliente, hay ciertas acciones que no pueden ser realizadas simultáneamente por múltiples procesos. En consecuencia, aparece una sección crítica al momento de acceder al archivo de apuestas. Por este motivo, se agregó un **Lock** de `multiprocessing` para sincronizar el acceso al archivo de apuestas, el cual permite que solo un proceso acceda a dicho recurso a la vez. Si más de un proceso escribe en el archivo simultáneamente, el resultado final no sería el esperado.

El Lock se usa para proteger el archivo de apuestas y asegurar que solo un proceso acceda a dicho recurso a la vez.

### Sincronización de procesos

Para gestionar las estructuras de datos compartidas entre los procesos, se utilizó `multiprocessing.Manager`.

Más específicamente, para sincronizar los procesos y garantizar que el servidor sepa cuándo todas las agencias han terminado de enviar sus apuestas, se utilizó un diccionario compartido proporcionado por `Manager` que puede ser accedido y modificado de manera segura por múltiples procesos. Este diccionario compartido reside en un espacio de memoria administrado por Manager, lo que permite que los cambios realizados por un proceso sean visibles para todos los demás.

En este caso, este diccionario, llamado `done_agencies`, permite que cada proceso registre de manera independiente cuándo una agencia ha finalizado su envío de apuestas.

Dado que `Manager` no soporta directamente conjuntos (set), se optó por usar un diccionario donde las claves representan las agencias y los valores son booleanos (_True_), indicando que la agencia correspondiente ha terminado.

Sin embargo, para garantizar la consistencia del diccionario compartido, es necesario sincronizar el acceso a este recurso. Esto se logra utilizando un `Lock` de `multiprocessing`. Sin el Lock, múltiples procesos podrían intentar escribir en el diccionario al mismo tiempo, lo que podría llevar a resultados inconsistentes o a un estado incorrecto del diccionario.

Cada proceso escribe únicamente el estado de su propia agencia en el diccionario, y luego se utiliza este recurso sincronizado para determinar cuándo todas las agencias han terminado. Una vez que todas las agencias están registradas como finalizadas, se puede proceder con el sorteo y enviar los resultados a los clientes al recibir el mensaje de **WINNERS_REQUEST**.

### Cómo ejecutarlo

La forma de ejecutarlo es la misma que la descrita en el ejercicio 7.
